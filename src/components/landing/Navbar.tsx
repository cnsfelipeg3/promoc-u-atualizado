              ...
              <AnimatePresence>
                {link.sub && openDropdown === link.label && (
                  <motion.div
                    ...
                    className="absolute top-full left-0 mt-1 w-48 bg-card border border-border rounded-xl shadow-lg overflow-hidden"
                  >
                    {link.sub.map((sub) => (
                      <a
                        key={sub.label}
          ...
          >
            <div className="px-4 py-4 space-y-1">
              {navLinks.map((link) => (
                <div key={link.label}>
                  <a
                    ...
                    className="block px-4 py-3 text-sm font-medium text-foreground/80 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors"
                  >
                    {link.label}
                  </a>
                  {link.sub && (
              ...
              <a
                href="#planos"
                onClick={() => setMobileOpen(false)}
                className="block glow-button text-sm text-center mt-4"
              >
                Assinar agora
              </a>
            </div>
