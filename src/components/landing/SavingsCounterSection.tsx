...
function FlipDigit({ digit, prev }: { digit: string; prev: string }) {
  ...
  return (
    <span className="relative inline-block w-[1ch] text-center overflow-hidden">
      {changed ? (
        <motion.span key={digit} initial={{ y: "100%", opacity: 0 }} animate={{ y: "0%", opacity: 1 }} transition={{ duration: 0.4, ease: "easeOut" }} className="inline-block">
          {digit}
        </motion.span>
      ) : (
        <span>{digit}</span>
        ...
        <ScrollReveal>
          <div className="glass-card-highlight p-8 sm:p-12 text-center relative overflow-hidden">
            <div className="flex items-center justify-center gap-2 mb-6">
              ...
              <p className="text-primary font-semibold text-sm uppercase tracking-widest font-mono">Total gerado em economias</p>
            </div>
            <AnalogCounter value={formatBRL(total)} prevValue={prevFormatted} />
            <div className="mt-6 flex items-center justify-center gap-2">
              <motion.span className="w-2 h-2 rounded-full bg-primary"
                animate={{ boxShadow: ["0 0 0px hsl(var(--primary) / 0)", "0 0 6px hsl(var(--primary) / 0.4)", "0 0 0px hsl(var(--primary) / 0)"] }}
                transition={{ duration: 1.5, repeat: Infinity }}
              />
              <span className="font-mono text-xs text-muted-foreground uppercase tracking-widest">Atualizando ao vivo</span>
            </div>
            <p className="text-muted-foreground text-sm mt-4 max-w-md mx-auto">
              Valor total economizado pelos membros da PromoCéu desde o lançamento.
            </p>
          </div>
        </ScrollReveal>
      </div>
